...
class Student
{
    ...
    public function student_list($page=1,$search_input=''){
       ...
       $student=array();
       if ($query->num_rows > 0) {
       while ($row = $query->fetch_assoc()) //Obtener una fila de resultado como un array asociativo 
       {
    ...
    $count_sql = "SELECT COUNT(*) FROM students $search"; // calcula el numero total de registros
    $query=  $this->conn->query($count_sql);
    $total = mysqli_fetch_row($query); // Obtener una fila de resultados como un array enumerado
    $student['total'][]= $total;
    ...
    public function update_student_info($post_data=array()){
       ...
       $sql="UPDATE students SET student_name='$student_name',email_address='$email_address',contact='$contact',country='$country',gender='$gender' WHERE student_id =$student_id";
        ...
        $result=  $this->conn->query($sql);
         ...
         unset($post_data->student_id); // unset = Destruye una variable especificada
         if($result){
    ...
    // destructor 
    function __destruct() {
    mysqli_close($this->conn);  // cerrar  consulta  
    }
